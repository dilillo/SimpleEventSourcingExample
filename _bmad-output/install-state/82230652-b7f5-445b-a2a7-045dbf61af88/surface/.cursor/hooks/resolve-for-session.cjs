...
var CURSOR_FACILITATOR_DEFINITION_SOURCE_PATH = ".cursor/agents/party-mode-facilitator.md";
var CLAUDE_FACILITATOR_TARGET_PATH = ".claude/agents/party-mode-facilitator.md";
...
function repoRoot() {
  ...
  const uniqueCandidates = [...new Set(candidates.map((candidate) => path2.resolve(candidate)))];
  for (const candidate of uniqueCandidates) {
    const marker = path2.resolve(candidate, REVIEWER_SHARED_CORE_METADATA_PATH);
...
function resolveRepoRelative(relativePath) {
  return path2.resolve(repoRoot(), relativePath);
}
function readJsonFile(relativePath) {
  ...
  return JSON.parse(fs2.readFileSync(absolutePath, "utf8"));
}
...
function packetArtifactPath(projectRoot, sessionId, packetId) {
  return path3.join(packetArtifactDir(projectRoot, sessionId), `${packetId}.json`);
}
...
function orchestrationStateDir(projectRoot) {
  return path4.join(projectRoot, "_bmad-output", "runtime", "governance", "orchestration-state");
}
function orchestrationStatePath(projectRoot, sessionId) {
  return path4.join(orchestrationStateDir(projectRoot), `${sessionId}.json`);
}
...
function writeOrchestrationState(projectRoot, state) {
  ...
  fs3.writeFileSync(tempFile, JSON.stringify(state, null, 2) + "\n", "utf8");
  fs3.renameSync(tempFile, file);
}
...
function resolveLocalizedMarkdownPath(input) {
  ...
  const resolvedMode = input.resolvedMode === "en" || input.resolvedMode === "zh" || input.resolvedMode === "bilingual" ? input.resolvedMode : "zh";
  if (resolvedMode === "en") {
...
var FACILITATOR_SOURCE_RELATIVE_PATHS = {
...
function resolveRelativeMarkdownAsset(projectRoot, relativePath, resolvedMode) {
  const resolved = resolveLocalizedMarkdownPath({
    basePath: path5.join(projectRoot, relativePath),
    resolvedMode
  });
  return {
    baseRelativePath: toPosix(relativePath),
    resolvedRelativePath: toPosix(path5.relative(projectRoot, resolved.resolvedPath)),
...
function resolveFacilitatorRuntimeBindings(projectRoot, host, resolvedMode) {
  return {
    ...
    step02: host === "cursor" ? resolveCursorPartyModeAsset(
      projectRoot,
...
function runtimeTargetRelativePath(host) {
  return host === "cursor" ? CURSOR_FACILITATOR_DEFINITION_SOURCE_PATH : CLAUDE_FACILITATOR_TARGET_PATH;
}
function detectMaterializedMode(projectRoot, explicitMode) {
  const ctxPathRelative = path6.relative(projectRoot, path6.join(projectRoot, "_bmad-output", "runtime", "context", "project.json")).replace(/\\/g, "/");
  if (explicitMode) {
    return {
      mode: explicitMode,
      contextPathRelative: ctxPathRelative,
      fallbackReason: explicitMode === "base" ? "explicit_base_override" : void 0
    };
  }
  const ctxPath = path6.join(projectRoot, "_bmad-output", "runtime", "context", "project.json");
  if (!fs5.existsSync(ctxPath)) {
    return {
      mode: "base",
      contextPathRelative: ctxPathRelative,
  ...
  try {
    const raw = JSON.parse(fs5.readFileSync(ctxPath, "utf8"));
...
function injectGeneratedHeader(content, metadata) {
  ...
  const header = `<!-- RUNTIME-MATERIALIZED facilitator resolvedMode=${metadata.mode} source=${metadata.sourceRelativePath} workflow=${metadata.workflowRelativePath} step01=${metadata.step01RelativePath} step02=${metadata.step02RelativePath} step03=${metadata.step03RelativePath} contextPath=${metadata.contextPathRelative}` + (metadata.fallbackReason ? ` fallbackReason=${metadata.fallbackReason}` : "") + " -->";
...
function resolveRuntimeBindings(projectRoot, host, mode) {
  if (mode === "base") {
    const facilitator = host === "cursor" ? "_bmad/cursor/agents/party-mode-facilitator.md" : "_bmad/claude/agents/party-mode-facilitator.md";
    return {
...
function materializeFacilitatorDefinition(projectRoot, host, mode, detectedMode = detectMaterializedMode(projectRoot, mode)) {
  const targetRelativePath2 = runtimeTargetRelativePath(host);
  const targetPath = path6.join(projectRoot, targetRelativePath2);
  const bindings = resolveRuntimeBindings(projectRoot, host, detectedMode.mode);
  const sourcePath = path6.join(projectRoot, bindings.facilitator.resolvedRelativePath);
  ...
  const source = fs5.readFileSync(sourcePath, "utf8");
  ...
  const materialized = injectGeneratedHeader(rewritten, {
    ...
    fallbackReason: detectedMode.mode === "base" ? detectedMode.fallbackReason : void 0
  });
  fs5.mkdirSync(path6.dirname(targetPath), { recursive: true });
  const previous = fs5.existsSync(targetPath) ? fs5.readFileSync(targetPath, "utf8") : null;
  ...
  fs5.writeFileSync(targetPath, materialized, "utf8");
  return {
    host,
    mode: detectedMode.mode,
    targetPath,
...
function ensureFacilitatorRuntimeDefinition2(projectRoot, options) {
  const detectedMode = detectMaterializedMode(projectRoot, options?.mode);
  const hosts = options?.hosts ?? ["cursor", "claude"];
  return hosts.map((host) => {
    const runtimeDir = host === "cursor" ? path6.join(projectRoot, ".cursor", "agents") : path6.join(projectRoot, ".claude", "agents");
    if (!fs5.existsSync(runtimeDir)) {
      return {
        host,
        mode: detectedMode.mode,
        targetPath: path6.join(projectRoot, runtimeTargetRelativePath(host)),
        fallbackReason: detectedMode.fallbackReason,
        updated: false,
        skippedReason: `runtime dir missing: ${path6.relative(projectRoot, runtimeDir).replace(/\\/g, "/")}`
      };
    }
...
// ../../scripts/reviewer-runtime-definition.ts
var fs6 = __toESM(require("node:fs"));
var path7 = __toESM(require("node:path"));
function sourceRelativePath(host) {
  return host === "cursor" ? CURSOR_REVIEWER_CANONICAL_SOURCE_PATH : CLAUDE_REVIEWER_CANONICAL_SOURCE_PATH;
}
function targetRelativePath(host) {
  return host === "cursor" ? CURSOR_REVIEWER_RUNTIME_TARGET_PATH : CLAUDE_REVIEWER_RUNTIME_TARGET_PATH;
}
function injectGeneratedHeader2(content, metadata) {
  const separator = content.includes("\r\n") ? "\r\n" : "\n";
  const header = `<!-- RUNTIME-MATERIALIZED reviewer source=${metadata.sourceRelativePath} shared_metadata=${REVIEWER_SHARED_CORE_METADATA_PATH} shared_profiles=${REVIEWER_SHARED_CORE_PROFILE_PACK_PATH} shared_prompt=${REVIEWER_SHARED_CORE_BASE_PROMPT_PATH} -->`;
  if (content.startsWith(`---${separator}`)) {
    const closingMarker = `${separator}---${separator}`;
    const closingIndex = content.indexOf(closingMarker, 4);
    if (closingIndex >= 0) {
...
function materializeReviewerDefinition(projectRoot, host) {
  const targetPath = path7.join(projectRoot, targetRelativePath(host));
  const sourceRelative = sourceRelativePath(host);
  const sourcePath = path7.join(projectRoot, sourceRelative);
  ...
  const source = fs6.readFileSync(sourcePath, "utf8");
  const materialized = injectGeneratedHeader2(stripGeneratedHeader(source), {
    sourceRelativePath: sourceRelative
  });
  fs6.mkdirSync(path7.dirname(targetPath), { recursive: true });
  const previous = fs6.existsSync(targetPath) ? fs6.readFileSync(targetPath, "utf8") : null;
  if (previous === materialized) {
    return {
      host,
      targetPath,
  ...
  fs6.writeFileSync(targetPath, materialized, "utf8");
  return {
    host,
    targetPath,
    sourceRelativePath: sourceRelative,
...
function ensureReviewerRuntimeDefinition2(projectRoot, options) {
  const hosts = options?.hosts ?? ["cursor", "claude"];
  return hosts.map((host) => {
    const runtimeDir = host === "cursor" ? path7.join(projectRoot, ".cursor", "agents") : path7.join(projectRoot, ".claude", "agents");
    if (!fs6.existsSync(runtimeDir)) {
      return {
        host,
        targetPath: path7.join(projectRoot, targetRelativePath(host)),
        updated: false,
        skippedReason: `runtime dir missing: ${path7.relative(projectRoot, runtimeDir).replace(/\\/g, "/")}`
      };
    }
...
function runtimeContextPath(root) {
  return path8.join(root, "_bmad-output", "runtime", "context", "project.json");
}
...
function resolveRuntimeContextPath(root, explicitPath) {
  ...
  if (candidate) {
    return path8.isAbsolute(candidate) ? candidate : path8.resolve(root, candidate);
  }
...
function mergeLanguagePolicyIntoProjectContext(root, languagePolicy) {
  ...
  ensureFacilitatorRuntimeDefinition2(root);
  ensureReviewerRuntimeDefinition2(root);
  return {
    status: "updated",
    contextPath: file
  };
...
function writeRuntimeContextFile(file, payload) {
  ...
  fs7.writeFileSync(tmp, body, "utf8");
  let fd = fs7.openSync(tmp, "r+");
  try {
    fs7.fsyncSync(fd);
  } finally {
    fs7.closeSync(fd);
...
function loadAllRecords(dataPath) {
  ...
  const entries = fs8.readdirSync(base, { withFileTypes: true });
  ...
  if (fs8.existsSync(jsonlPath)) {
    const lines = fs8.readFileSync(jsonlPath, "utf-8").split("\n").map((l) => l.trim()).filter((l) => l.length > 0);
...
// ../../scripts/prompt-routing-hints.ts
var DEFAULT_PROMPT_ROUTING_RULES_PATH = path11.join(
  "_bmad",
  "bmm",
  "data",
...
function governanceRemediationConfigPath(projectRoot) {
  return path12.join(projectRoot, "_bmad", "_config", "governance-remediation.yaml");
}
...
function readGovernanceRemediationConfig(projectRoot, explicitPath) {
  const file = explicitPath ? path12.isAbsolute(explicitPath) ? explicitPath : path12.resolve(projectRoot, explicitPath) : governanceRemediationConfigPath(projectRoot);
  if (!fs9.existsSync(file)) {
    return defaultGovernanceRemediationConfig();
  }
  const parsed = load(fs9.readFileSync(file, "utf8"));
...
function sharedPathScore(left, right) {
  const leftSegments = pathSegments(path13.normalize(left).toLowerCase());
  const rightSegments = pathSegments(path13.normalize(right).toLowerCase());
  let score = 0;
...
function listScopedOrchestrationStatePaths(projectRoot) {
  ...
  return fs10.readdirSync(dir).filter((file) => file.endsWith(".json")).map((file) => path13.join(dir, file));
}
function resolveScopedOrchestrationState(projectRoot, runtimeContext) {
  ...
  const scored = candidates.map((candidate) => {
    const sessionId = path13.basename(candidate, ".json");
    ...
    return {
      ...
      state,
      score,
      mtimeMs: fs10.statSync(candidate).mtimeMs
    };
...
function readPendingPacketPayload(state) {
  ...
  if (!packetPath || !fs10.existsSync(packetPath)) {
    return null;
  }
  try {
    return JSON.parse(fs10.readFileSync(packetPath, "utf8"));
  } catch {
    return null;
  }
}
function normalizePendingPacketStatus(state, packetPayload) {
  ...
  if (!packetPayload && state.pendingPacket.packetPath && !fs10.existsSync(state.pendingPacket.packetPath)) {
...
function deriveSessionIdFromRuntimeContext(flow, runtimeContext) {
  ...
  if (artifactRoot) {
    return `${flow}-${path13.basename(artifactRoot).replace(/[^a-zA-Z0-9._-]+/g, "-")}`;
  }
  return `${flow}-session`;
}
...
function writePacketFile(projectRoot, sessionId, packetId, packet) {
  ...
  fs10.writeFileSync(file, JSON.stringify(packet, null, 2) + "\n", "utf8");
  return file;
}
...
function deriveDriftSurface(projectRoot, closeout) {
  ...
  try {
    const records = loadAndDedupeRecords(path13.join(projectRoot, "packages", "scoring", "data"));
...
function ensureMainAgentDispatchPacket(input) {
  ...
  if (currentSurface.orchestrationState) {
    ...
    const file = path13.join(orchestrationStateDir(input.projectRoot), `${sessionId}.json`);
    fs10.mkdirSync(path13.dirname(file), { recursive: true });
    fs10.writeFileSync(file, JSON.stringify(writtenState, null, 2) + "\n", "utf8");
  }
  ...
  return {
    source: "orchestration_state",
    sessionId,
    orchestrationStatePath: path13.join(orchestrationStateDir(input.projectRoot), `${sessionId}.json`),
...
function pickRoot(args) {
  ...
  return fromArg ? path13.resolve(fromArg) : process.cwd();
}
...
function resolveSessionAndPacketFromSurface(surface, args) {
  ...
  const packetId = normalizeText(args.packetId) || normalizeText(surface.orchestrationState?.pendingPacket?.packetId);
...
function loadConfigFromFile(configPath) {
  const path15 = configPath || DEFAULT_CONFIG_PATH;
  if (!(0, import_node_fs.existsSync)(path15)) {
    return {};
  }
  try {
    const content = (0, import_node_fs.readFileSync)(path15, "utf8");
    return load(content);
  } catch (error) {
    console.error(`[bmad-config] \u8BFB\u53D6\u914D\u7F6E\u6587\u4EF6\u5931\u8D25: ${path15}`, error);
    return {};
  }
}
...
async function main() {
  ...
  const projectRoot = path14.resolve(String(body.projectRoot || process.cwd()));
  ...
  try {
    ...
    const receipts = writeContext ? ensureFacilitatorRuntimeDefinition2(projectRoot, {
    ...
    if (writeContext) {
      ...
      if (out.contextSync && typeof out.contextSync === "object" && out.contextSync.status === "skipped") {
        out.temporaryResolvedModeApplied = {
          resolvedMode: policy.resolvedMode,
          targets: receipts.filter((receipt) => receipt.skippedReason == null).map((receipt) => ({
            host: receipt.host,
